...
struct NodeLeftExtension {
    ...
    uint64_t first_l(CharId l){
        ...
        return nodes[l].firsts[0];
    }

    uint64_t first_r(CharId r){
        uint64_t min = std::numeric_limits<uint64_t>::max();
        for(int i=0; i<nodes.size(); i++){
            if(nodes[i].firsts[r]<min){
                min = nodes[i].firsts[r];
            }
        }
        return min;
    }
    ...
    vector<tuple<CharId, CharId, uint64_t>> distinct_extensions(){
        ...
        for(int i=0; i<nodes.size(); i++){
            for(auto r: nodes[i].distinct_extensions()){
    ...
    bool dup_sequence_exists(){
        return nodes[0].suffix_count()>1;
    }

    bool left_maximal(){
        int distinct = 0;
        for(auto interval:nodes){
            if(interval.count()>0){
                distinct++;
            }
        }
...
*/
NodeLeftExtension extend_left(FmIndex &index, SuffixArrayNode interval){
    ...
    vector<SuffixArrayNode> left_nodes;
    for(int c=0; c < index.STRING->get_characters().size(); c++){
        ...
        left_nodes.push_back({firsts, interval.depth+1});
    }
