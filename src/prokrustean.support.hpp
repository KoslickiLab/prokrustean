...
void count_overlap_based_coverage(ProkrusteanExtension &ext, vector<uint8_t> &incoming_degrees, vector<uint32_t> &overlap_degrees_out, bool allow_multi_edge){
    ...
    vector<bool> strata_containing_symmetric_prefsuff(ext.prokrustean.stratum_count, 0);
    ...
    */ 
    ...
    for(SeqId i=0; i<ext.prokrustean.sequence_count; i++){
        ...
        for(CoveringRegionIdx j=0; j<vertex.s_edges.size(); j++){
            ...
            if(j==0 && s_edge.from==0){
                // if(i==896){
                //     cout << "896 prefix stratum length " << s_edge.stratum_id << " " << s_edge.size() << endl;
                // }
                strata_prefix_occurrences[s_edge.stratum_id]+=1;
            }
            if(j+1==vertex.s_edges.size() && s_edge.to==vertex.size){
                // if(i==896){
                //     cout << "896 suffix stratum length " << s_edge.stratum_id << " " << s_edge.size() << endl;
                // }
    ...
    for(SeqId i=0; i<ext.prokrustean.sequence_count; i++){
        if(ext.get_sequence_first_stratified(i, edge) && edge.from==0){
            ...
            while(true){
                // if(i==896){
                //     cout << "seq 896 front " << stratum_id << " " << ext.prokrustean.get_stratum_size(stratum_id) << " " << strata_suffix_occurrences[stratum_id]<< endl;
                // }
                if(allow_multi_edge){
                    // this means overlap will be computed multiple times for certain pairs
                    overlap_degrees_out[i]+=strata_suffix_occurrences[stratum_id];
        ...
        if(ext.get_sequence_first_stratified(i, edge) && edge.size()==ext.prokrustean.sequences__size[i]){
            overlap_degrees_out[i]-=2;
        }
    }
}

// void count_overlap_based_coverage(ProkrusteanExtension &ext, vector<uint8_t> &incoming_degrees, vector<uint32_t> &overlap_degrees_out){
//     overlap_degrees_out.resize(ext.prokrustean.sequence_count, 0);
//     vector<uint32_t> strata_prefix_occurrences(ext.prokrustean.stratum_count, 0);
//     vector<uint32_t> strata_suffix_occurrences(ext.prokrustean.stratum_count, 0);
//     vector<optional<uint32_t>> strata_front_overlaps(ext.prokrustean.stratum_count);
//     vector<optional<uint32_t>> strata_back_overlaps(ext.prokrustean.stratum_count);
...
//     for(SeqId i=0; i<ext.prokrustean.sequence_count; i++){
//         if(i<100){
//             cout << "i: " << i << " prefix: " << strata_prefix_occurrences[i] << " suffix: " << strata_suffix_occurrences[i] << endl;
//         }
//     }
//     // for(auto occ:strata_prefix_occurrences){
//     //     cout << "prefix " << occ << endl;
//     // }
//     // for(auto occ:strata_suffix_occurrences){
//     //     cout << "suffix " << occ << endl;
//     // }

//     /*
//     Compute accumulated overlaps for each stratum, and then find the final overlap degree of each sequence 
//     */ 
//     StratifiedEdge edge;
//     stack<StratumId> higher_strata;
//     StratumId child_stratum_id;
//     for(SeqId i=0; i<ext.prokrustean.sequence_count; i++){
//         if(ext.get_sequence_first_stratified(i, edge)){
//             stratum_id=edge.stratum_id;
//             if(!strata_front_overlaps[edge.stratum_id].has_value()){
//                 while(true){
//                     // compose hierarchy and set the front overlaps for the lowest stratum.
//                     if(ext.get_stratum_first_stratified(stratum_id, edge) && edge.from==0){
//                         if(strata_front_overlaps[edge.stratum_id].has_value()){
//                             strata_front_overlaps[stratum_id]=strata_front_overlaps[edge.stratum_id].value()+strata_suffix_occurrences[edge.stratum_id];
//                             break;
//                         } else {
//                             higher_strata.push(stratum_id);
//                             stratum_id=edge.stratum_id; 
//                         }
//                     } else {
//                         strata_front_overlaps[stratum_id]=0;
//                         break;
//                     }
//                 }
                
//                 while(!higher_strata.empty()){
//                     child_stratum_id=stratum_id;
//                     stratum_id=higher_strata.top();
//                     higher_strata.pop();

//                     assert(strata_front_overlaps[child_stratum_id].has_value());
//                     strata_front_overlaps[stratum_id]=strata_front_overlaps[child_stratum_id].value()+strata_suffix_occurrences[child_stratum_id];
//                 }
//             }

//             assert(strata_front_overlaps[stratum_id].has_value());
//             // cout << "strata_suffix_occurrences[stratum_id] " << strata_suffix_occurrences[stratum_id] << " stratum_id " << stratum_id << endl;
//             overlap_degrees_out[i]+=strata_front_overlaps[stratum_id].value()+strata_suffix_occurrences[stratum_id];
//         }
        
//         if(ext.get_sequence_last_stratified(i, edge)){
//             stratum_id=edge.stratum_id;
//             if(!strata_back_overlaps[edge.stratum_id].has_value()){
//                 while(true){
//                     // compose hierarchy and set the front overlaps for the lowest stratum.
//                     if(ext.get_stratum_last_stratified(stratum_id, edge) && edge.to==ext.prokrustean.stratums__size[stratum_id]){
//                         if(strata_back_overlaps[edge.stratum_id].has_value()){
//                             strata_back_overlaps[stratum_id]=strata_back_overlaps[edge.stratum_id].value()+strata_prefix_occurrences[edge.stratum_id];
//                             break;
//                         } else {
//                             higher_strata.push(stratum_id);
//                             stratum_id=edge.stratum_id; 
//                         }
//                     } else {
//                         strata_back_overlaps[stratum_id]=0;
//                         break;
//                     }
//                 }
...
//                     assert(strata_back_overlaps[child_stratum_id].has_value());
//                     strata_back_overlaps[stratum_id]=strata_back_overlaps[child_stratum_id].value()+strata_prefix_occurrences[child_stratum_id];
//                 }
//             }

//             assert(strata_back_overlaps[stratum_id].has_value());
//             // cout << "strata_back_overlaps[stratum_id].value() " << strata_back_overlaps[stratum_id].value() << " strata_prefix_occurrences[stratum_id] " << strata_prefix_occurrences[stratum_id] << endl; 
//             overlap_degrees_out[i]+=strata_back_overlaps[stratum_id].value()+strata_prefix_occurrences[stratum_id];
//         }
//     }
// }
