...
public:
    size_type const & sigma = m_sigma;        //!< Effective alphabet size of the wavelet tree.
    bit_vector_type const & tree = m_tree;    //!< A concatenation of all bit vectors of the wavelet tree.
    uint32_t const & max_level = m_max_level; //!< Maximal level of the wavelet tree.
    ...
    wt_int(t_it begin, t_it end, std::string tmp_dir = ram_file_name("")) : m_size(std::distance(begin, end))
    {
        ...
        std::string tree_out_buf_file_name = tmp_file(tmp_dir, "_m_tree");
        {
            ...
            for (uint32_t k = 0; k < m_max_level; ++k)
            {
                ...
                do
                {
                    ...
                    while (i < m_size and ((x = rac[i]) & mask_old) == start_value)
                    {
                        ...
                        { // if tree_pos % 64 == 0 write old word
            ...
            if ((tree_pos & 0x3FULL) != 0)
            { // if tree_pos % 64 > 0 => there are remaining entries we have to write
                tree_out_buf.write((char *)&tree_word, sizeof(tree_word));
            }
    ...
    value_type operator[](size_type i) const
    {
        ...
        for (uint32_t k = 0; k < m_max_level; ++k)
        {
            ...
            size_type ones_before_end = m_tree_rank(offset + node_size) - ones_before_o;
            if (m_tree[offset + i])
            { // one at position i => follow right child
            ...
            { // zero at position i => follow left child
    ...
    size_type rank(size_type i, value_type c) const
    {
        ...
        for (uint32_t k = 0; k < m_max_level and i; ++k)
        {
            ...
            size_type ones_before_end = m_tree_rank(offset + node_size) - ones_before_o;
            if (c & mask)
            { // search for a one at this level
                offset += (node_size - ones_before_end);
                node_size = ones_before_end;
                i = ones_before_i;
            }
            else
            { // search for a zero at this level
                node_size = (node_size - ones_before_end);
                i = (i - ones_before_i);
            }
            offset += m_size;
