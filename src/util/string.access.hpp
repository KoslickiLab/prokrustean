...
struct ProkrusteanSequenceMetadata {
    bool loaded;
    std::string evidence;
    std::string bwt_file_name;
    uint64_t sequence_count;
    uint8_t sequence_size_type_byte_size;
    vector<std::streampos> streampos_sequences; 

    void print(){
        cout << " meta: " << endl;
        cout << " evidence " << evidence << endl;
        cout << " bwt_file_name " << bwt_file_name << endl;
        cout << " sequence_count " << sequence_count << endl;
    }
};
...
public:
    ...
    void write_metadata(vector<SequenceSize> &seq_sizes) {
        assert(this->writefile.is_open());
        ////////////////////////////////////////////////
        // std::string evidence;
        // std::string prokrustean_file_name;
        ...
        this->metadata.loaded=true;
        this->metadata.evidence=PROKRUSTEAN_EVIDENCE;
        this->metadata.bwt_file_name=this->filename;
        this->metadata.sequence_size_type_byte_size=sizeof(SequenceSize);
        this->metadata.sequence_count=seq_sizes.size();
        this->metadata.streampos_sequences.resize(seq_sizes.size());
        ////////////////////////////////////////////////////////////////////////////////////////////////
        // content                              // bytes                                              //
        ////////////////////////////////////////////////////////////////////////////////////////////////
        // PROKRUSTEAN_EVIDENCE                 // 256
        // bwt_file_name                        // 256
        // sequence_size_type_bytes             // 1
        // sequence_count                       // 8
        // streampos_sequences                  // sizeof(streampos) * sequence count
        ////////////////////////////////////////////////////////////////////////////////////////////////
        this->metadata.evidence.resize(256);
        writefile.write(this->metadata.evidence.c_str(), 256);
        this->metadata.bwt_file_name.resize(256);
        writefile.write(this->metadata.bwt_file_name.c_str(), 256);
        writefile.write(reinterpret_cast<const char*>(&this->metadata.sequence_count), sizeof(this->metadata.sequence_count));
        writefile.write(reinterpret_cast<const char*>(&this->metadata.sequence_size_type_byte_size), sizeof(this->metadata.sequence_size_type_byte_size));
        // jump the number of sequences
        std::streampos seq_pointer = writefile.tellp(); 
        seq_pointer += this->metadata.sequence_count * sizeof(std::streampos);
        for(uint64_t i=0; i<this->metadata.sequence_count; i++){
            this->metadata.streampos_sequences[i]=seq_pointer;
            writefile.write(reinterpret_cast<const char*>(&seq_pointer), sizeof(std::streampos));    
            seq_pointer += sizeof(SequenceSize) + seq_sizes[i];
        }
    }
    ...
    void write_strings(const std::vector<std::string>& strings) {
        ...
        // Placeholder for string positions
        for (uint64_t i = 0; i < strings.size(); ++i) {
            assert(this->metadata.streampos_sequences[i]==writefile.tellp());
            SequenceSize len = strings[i].size();
            writefile.write(reinterpret_cast<const char*>(&len), sizeof(len));
            writefile.write(strings[i].c_str(), len);
        }
    }
    ...
    void load_metadata() {
        ...
        char bwt_file_name_buffer[256];
        _infile.read(bwt_file_name_buffer, 256);
        metadata.bwt_file_name = std::string(bwt_file_name_buffer, 256);

        _infile.read(reinterpret_cast<char*>(&metadata.sequence_count), sizeof(metadata.sequence_count));
        _infile.read(reinterpret_cast<char*>(&metadata.sequence_size_type_byte_size), sizeof(metadata.sequence_size_type_byte_size));
        if(metadata.sequence_size_type_byte_size!=sizeof(SequenceSize)){
            cout << "Error: type SequenceSize may have changed from the original data. original data uses " << metadata.sequence_size_type_byte_size << " bytes" << " but currently " << sizeof(SequenceSize) << " bytes" << endl;
        }
        this->metadata.streampos_sequences.resize(this->metadata.sequence_count);
        for(uint64_t i=0; i<this->metadata.sequence_count; i++){
            _infile.read(reinterpret_cast<char*>(&this->metadata.streampos_sequences[i]), sizeof(std::streampos));
        }
        _infile.close();
        metadata.loaded=true;
    }
    ...
    void read_seq(SeqId id, std::string &sequence){
        ...
        assert(metadata.loaded);
        assert(id<metadata.sequence_count);
        infile.seekg(metadata.streampos_sequences[id]);
        SequenceSize len;
        infile.read(reinterpret_cast<char*>(&len), sizeof(len));
        sequence.resize(len);
        infile.read(&sequence[0], len);
    ...
    void load_all_strings() {
        std::ifstream _infile(filename, std::ios::binary);
        _infile.seekg(metadata.streampos_sequences[0]);
