...
void _refracted_contributions_when_no_stratified_exists(Vertex vertex, int from, vector<int64_t> &partial_partial_C){
    ...
    if(vertex.s_edges.size()==0){
        ...
        for(auto s: vertex.s_edges){
            max_s_length=std::max<StratumSize>(max_s_length, s.size());
        }
        auto k_mer_starts_decreasing_at=std::max<StratumSize>(from+1, 2+max_s_length);
...
void _refracted_contributions_between_two_strata(Vertex vertex, int from, vector<int64_t> &partial_partial_C){
    ...
    for(int i=rgn_cnt-2; i>=0; i--){
        maximum_length_on_right=std::max<StratumSize>(maximum_length_on_right, vertex.s_edges[i+1].size());
        ...
        if(vertex.s_edges[i].to<=vertex.s_edges[i+1].from){
            // no intersection
            refracted_appears_at=std::max<SequenceSize>(2, from+1);
        } else {
            // intersection
            refracted_appears_at=std::max<SequenceSize>(vertex.s_edges[i].to - vertex.s_edges[i+1].from + 2, from + 1);
        }
...
void count_distinct_kmers_of_range(uint64_t from, uint64_t to, Prokrustean &prokrustean, vector<uint64_t> &output){
    ...
    for(int i=0; i<prokrustean.sequence_count; i++){
        ...
        _refracted_contributions_when_no_stratified_exists(sequence, from, partial_partial_C);
        _refracted_contributions_between_two_strata(sequence, from, partial_partial_C);
    }

    for(int i=0; i<prokrustean.stratum_count; i++){
        auto stratum=prokrustean.get_stratum(i, from);
        output[from]+=_count_k_mers(stratum, from);
        _refracted_contributions_when_no_stratified_exists(stratum, from, partial_partial_C);
        _refracted_contributions_between_two_strata(stratum, from, partial_partial_C);
    }
