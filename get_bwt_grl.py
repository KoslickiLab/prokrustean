...
def main(input):
    ...
    in_path_for_concatenated_string = in_path_for_fastq_file.replace(".fastq.gz", ".txt")
    # grlbwt processed. they don't like underscore ex. "./downloads/xxx.txt"
    out_path_grlbwt_rl_bwt_file = in_path_for_fastq_file.replace(".fastq.gz", "").replace("_", "")
    ...
    def process_fastq_gz(file_path, output_file_path):
